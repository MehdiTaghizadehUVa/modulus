...
class HydroGraphDataset(DGLDataset):
    ...
    If return_physics is True, each sample returns a tuple (graph, physics_data) where physics_data
    contains physics-related scalars (in the denormalized domain) plus necessary normalization parameters.
    Otherwise, only the DGLGraph is returned.
    """
    ...
    def __init__(self, name: str = "hydrograph_dataset", data_dir: Union[str, Path] = "data_directory",
                 ...
                 split: str = "train", force_reload: bool = False, verbose: bool = False,
    ...
    def __getitem__(self, idx: int):
        ...
        if self.return_physics:
            ...
            avg_inflow = float((dyn["inflow_hydrograph"][prev_time] + dyn["inflow_hydrograph"][target_time]) / 2)
            avg_precipitation = float((dyn["precipitation"][prev_time] + dyn["precipitation"][target_time]) / 2)
            # Denormalize static infiltration using its normalization parameters.
            infiltration_denorm = self.denormalize(sd["infiltration"],
                                                   self.static_stats["infiltration"]["mean"],
                                                   self.static_stats["infiltration"]["std"])
            # Compute the aggregated infiltration-area term over all nodes:
            infiltration_area_sum = float(np.sum(infiltration_denorm * sd["area_denorm"])) / 100.0
            physics_data = {
                "flow_hydrograph": float(future_inflow),
                ...
                "infiltration_area_sum": infiltration_area_sum
            }
    ...
    def create_node_features(self, xy_coords: np.ndarray, area: np.ndarray, elevation: np.ndarray,
                             ...
                             inflow_hydrograph: np.ndarray) -> (np.ndarray, float):
