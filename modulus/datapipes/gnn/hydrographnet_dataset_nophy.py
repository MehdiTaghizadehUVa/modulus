#!/usr/bin/env python
"""
Module: hydrograph_dataset_module.py

This module provides utilities for downloading and verifying large datasets from the web
(e.g., via Zenodo) and defines the HydroGraphDataset class for processing hydrograph data.
It includes:
  - Robust file download utilities with MD5 checksum verification.
  - Archive extraction for tar/zip files.
  - A function to download files from a Zenodo record.
  - The HydroGraphDataset class, a subclass of DGLDataset, for creating graph-based datasets.

Before using the HydroGraphDataset, ensure that the data is available locally by calling
the ensure_data_available() function with the desired data directory.
"""
...
class HydroGraphDataset(DGLDataset):
    ...
    When using the "pushforward" noise type, the dataset retrieves a dynamic window of
    size n_time_steps+1 (i.e. one extra step) so that the trainer can perform the pushforward
    unrolling. However, the model input will consist only of the last n_time_steps dynamic steps.
    """
    ...
    def __init__(
        ...
        self.noise_type = noise_type  # store noise type
    ...
    def process(self) -> None:
        ...
        # Added tqdm here to track progress when loading hydrograph data.
        temp_dynamic_data = []
        water_depth_list = []
        volume_list = []
        precipitation_list = []
        ...
        # Build sample index.
        # For pushforward, we need n_time_steps+1 dynamic steps available.
    ...
    def __getitem__(self, idx: int) -> dgl.DGLGraph:
        ...
        # Determine slice length: for pushforward, use n_time_steps+1 dynamic values,
        # for other noise types, use n_time_steps.
        if self.noise_type == "pushforward":
            end_index = t_idx + self.n_time_steps + 1
        else:
        ...
        node_features, future_inflow = self.create_node_features(
            ...
            dyn["precipitation"],
            t_idx,  # not used inside create_node_features directly
            self.n_time_steps,
            dyn["inflow_hydrograph"]
        )
    ...
    @staticmethod
    def normalize(data: np.ndarray, mean: Union[float, list, np.ndarray], std: Union[float, list, np.ndarray],
    ...
    @staticmethod
    def denormalize(data: np.ndarray, mean: Union[float, list, np.ndarray], std: Union[float, list, np.ndarray],
                    epsilon: float = 1e-8) -> np.ndarray:
        mean = np.array(mean) if isinstance(mean, list) else mean
        std = np.array(std) if isinstance(std, list) else std
    ...
    def load_dynamic_data(self, folder: str, hydrograph_id: str, prefix: str, num_points: int, interval: int = 1,
                          skip: int = 72):
        ...
        precipitation = precipitation[:peak_time_idx + 25] * 2.7778e-7
